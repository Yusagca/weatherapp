...
const getBackgroundVideo = (code: number) => {
  if (code === 0) {
    return 'public/backgrounds/clear.mov'; // Açık Hava
  } else if ((code >= 1 && code <= 3)) {
    return 'public/backgrounds/cloudy.mov'; // Sisli veya Bulutlu
  }else if ((code >= 45 && code <= 48)) {
    return 'public/backgrounds/fog.mov'; // Sisli veya Bulutlu
  }
   else if (
    ...
    return 'public/backgrounds/rain.mov'; // Yağmurlu
  } else if (
    (code >= 71 && code <= 77) || // Hafif-Orta Şiddetli Kar Yağışı
    (code >= 85 && code <= 86) || // Kar Sağanağı
    (code >= 96 && code <= 99)    // Şiddetli Fırtına ve Kar
  ) {
    return 'public/backgrounds/snow.mov'; // Karlı
  } else {
    return 'public/backgrounds/clear.mov'; // Varsayılan
  }
};
